...
export async function loadCalibrationSpots(): Promise<LoadedSpots> {
  ...
  const { data, error } = await supabase
    .from('spots')
    .select(
      'id, spot_type, pillar, template_id, template_payload, hole_cards, board, pot_size, villain_action, prompt, hero_position, correct_answer, is_catastrophic_if_wrong, sequence_order'
    )
