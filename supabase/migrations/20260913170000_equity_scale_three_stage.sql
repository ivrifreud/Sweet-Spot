-- ============================================================================
-- Equity Scale three-stage grading: weighted Elo from outs, equity, and
-- call/fold. Chip burns stay tied to the decision only.
-- ============================================================================
...
declare
  ...
  v_selected_equity numeric;
  v_correct_outs integer;
  v_true_equity integer;
  v_street text;
  v_outs_ok boolean := false;
  v_equity_ok boolean := false;
begin
  ...
  if p_answer_metadata ? 'selectedEquity' then
    if jsonb_typeof(p_answer_metadata->'selectedEquity') <> 'number' then
      raise exception 'selectedEquity must be numeric';
    end if;
    v_selected_equity := (p_answer_metadata->>'selectedEquity')::numeric;
    if v_selected_equity <> trunc(v_selected_equity)
      or v_selected_equity < 0
      or v_selected_equity > 100
    then
      raise exception 'selectedEquity must be an integer from 0 to 100';
    end if;
  end if;
  ...
  if v_spot.template_id = 2
    and v_selected_outs is not null
    and v_selected_equity is not null
  then
    v_street := v_spot.template_payload->>'street';
    v_correct_outs := (v_spot.template_payload->>'correctOuts')::integer;
    if v_street = 'turn' then
      v_true_equity := round((v_correct_outs::numeric / 46.0) * 100.0)::integer;
    else
      v_true_equity := round(
        (
          1.0
          - ((47.0 - v_correct_outs) / 47.0)
          * ((46.0 - v_correct_outs) / 46.0)
        ) * 100.0
      )::integer;
    end if;
    v_outs_ok := abs(v_selected_outs - v_correct_outs) = 0;
    v_equity_ok := abs(v_selected_equity - v_true_equity) <= 2;
    v_score :=
      (case when v_is_correct then 0.5 else 0.0 end)
      + (case when v_outs_ok then 0.25 else 0.0 end)
      + (case when v_equity_ok then 0.25 else 0.0 end);
  end if;
...
as $$
begin
  return public.submit_stage_answer(
    p_stage_progress_id,
    p_spot_id,
    p_chosen_answer,
    '{}'::jsonb
