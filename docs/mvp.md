...
# SWEET SPOT — MVP Feature Specification
...
1. [Introduction & MVP Scope](#1-introduction--mvp-scope)
...
10. [Explicitly Out of Scope for MVP](#10-explicitly-out-of-scope-for-mvp)
11. [Open Items Still Needing Input](#11-open-items-still-needing-input)
12. [Data Collection & Analytics Plan (Summary)](#12-data-collection--analytics-plan-summary)
13. [Business Model & Monetization Strategy (Summary)](#13-business-model--monetization-strategy-summary)

Map / engine (inserted in the source PDF after templates; not given its own TOC number there): [Map Content Plan & Spot Engine Specification](#map-content-plan--spot-engine-specification)
...
Levels 4–5, the Sweet Spot Arena (real-time PvP), the GTO Lab, and most of the deeper monetization surface are explicitly deferred — see [Section 10](#10-explicitly-out-of-scope-for-mvp).
...
A residual misplacement doesn't need a dedicated contest UI. The Elo Gap Multiplier ([Section 9](#9-shared-systems)) already penalizes mismatched difficulty asymmetrically, so a wrong placement self-corrects within a handful of real stages. The only added safety valve is a manual “Retake Placement Test” option in settings.
...
Five of the six pillars are directly relevant to Levels 1–3 and ship in MVP. Pillar VI is exclusively Level 4+ content and is deferred alongside that rollout — see [Section 10](#10-explicitly-out-of-scope-for-mvp).
...
## Map Content Plan & Spot Engine Specification

### The Six Pillars (Curriculum Architecture)

The overarching world paths are organized into the following foundational pillars:

- **Pillar I — Pre-Flop Architecture:** Foundational hand selection and positional discipline (UTG vs. Button dynamics).
- **Pillar II — Mathematical Framework:** Moving from intuition to EV+ decisions and pot-odds math.
- **Pillar III — Board Texture & Range Analysis:** Reading wet vs. dry boards; thinking in opponent ranges, not just own cards.
- **Pillar IV — Bet Sizing Geometry:** Sizing precision relative to pot, replacing random intuitive bets.
- **Pillar V — Opponent Profiling & Exploitation:** Identifying opponent archetypes and adjusting strategy to exploit them.
- **Pillar VI — High-Stakes Dynamics & ICM:** Push/fold strategy and tournament-bubble ICM pressure (Deferred for MVP, Levels 4–5).

### Content Work: Static Architecture & Path Progression

To define exactly what content each user sees, the map utilizes a hybrid model of a static curriculum skeleton populated by dynamic gameplay:

- **World Paths & Meaningful Splitting:** The visual world paths will be divided into the respective Pillars based on user level and timing. Paths will split and join dynamically. Specifically, if a user repeatedly fails a concept and loses their Chips, the path will visually branch into a "remedial detour" that breaks down the underlying math more simply before merging back onto the main track.
- **Static Lessons:** Within each Pillar, there will be a predefined, chronological list of static lessons dictating a specific field of study and the strategic concept being taught.
- **Static Sub-Lessons (The 7 Spots):** Underneath each lesson are 7 static sub-lessons (spots) that map directly to the curriculum's "sandwich structure":
  - **Spot 1 (Warm-up):** An easy win to build momentum, reviewing a previous concept.
  - **Spots 2–3 (New Concept):** Introducing the specific sub-lesson theory with highly obvious right/wrong margins.
  - **Spots 4–6 (Rising Difficulty):** Practice scenarios where the EV margins get progressively tighter.
  - **Spot 7 (Final Challenge):** A summarizing "boss" spot that requires perfectly executing the concept without hints.
- **Content Work Requirements:** Work will involve defining the chronological list of lessons and mapping out the static inputs (names, descriptions, constraint rules, and allowed UI templates) for every sub-lesson across the tracks.

### The Spot Engine (Dynamic Generation)

While the curriculum's structure is static, the actual gameplay content within each sub-lesson is procedurally generated by the Spot Engine.

- **Engine Inputs:** The engine will ingest both Static Parameters (sub-lesson name, learning objective, allowed UI templates) and Dynamic Parameters (the user's Hidden Elo). Feeding the Elo into the engine ensures that two users on the exact same spot face appropriately scaled margins of error (e.g., tighter math for a high-Elo user, forgiving setups for a lower-Elo user).
- **Constraint Rules (Rigid Boundaries):** To prevent the engine from generating obscure or unsolvable edge-cases, the content map will define strict bounding boxes for each sub-lesson (e.g., "Villain stack size must be > 40bb," or "Board cannot contain 4 to a flush").
- **Dynamic Output & Replay Logic:** Using these parameters, the Spot Engine will generate a unique, suitable poker scenario every time the user enters the spot to test them on that specific subject. If a user fails a spot and burns a Chip, the engine will generate a brand-new hand within the same parameters on their next attempt. This ensures the engine tests true comprehension rather than rote memory of the previous failure.
...
- Purchased cosmetic sets (chip skins, card backs) carry across all Worlds for personal expression — ties into [Section 13](#13-business-model--monetization-strategy-summary)'s monetization plan.
...
> **Scope Decision**  
> The MVP's one multiplayer feature is the Daily Challenge: single-player, no live opponent, no matchmaking infrastructure. The separate real-time PvP “Sweet Spot Arena” (Mirror Spots, The Duel, Sudden Death) is deferred — see [Section 10](#10-explicitly-out-of-scope-for-mvp).
...
| System | Role |
| --- | --- |
| Hidden Elo Rating | Backbone of level placement ([Section 2](#2-onboarding--the-adaptive-calibration-engine)) and in-track difficulty; updated only by standard-stage performance, never by Daily Challenge or Bankroll. |
...
- Validate the 5–6 hands/stage figure in the Calibration Engine ([Section 2](#2-onboarding--the-adaptive-calibration-engine)) with content/UX before production.
...
| Needs legal review | Real-money poker-room affiliate deals and aggregated data licensing — both intersect directly with the responsible-gambling signals in [Section 12](#12-data-collection--analytics-plan-summary). |
...
SweetSpot MVP.pdf · Sweet Spot – Psychology, Retention & Monetization Strategy.docx · Multiplayer Concepts Specification.docx · Session Times, Progression & Punishment System.pdf · Players Ranking and Elo.docx · The Six Pillars and the Adaptive Calibration Engine.docx · הרחבה של העולמות.docx · Question Templates.docx · companion documents “Data Collection & Analytics Plan” and “Business Model & Monetization Strategy” — plus scope decisions confirmed with the product owner through 2026-08-18.
